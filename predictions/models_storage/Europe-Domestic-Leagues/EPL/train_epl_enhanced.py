...
DATA_PATH = os.path.join(os.path.dirname(__file__), "epldata.csv")
MODELS_DIR = os.path.join(os.path.dirname(__file__), "models")
MODEL_PATH = os.path.join(MODELS_DIR, "best_model_neural_network.pkl")
...
def load_dataset(path: str = DATA_PATH, include_2024_25: bool = True) -> pd.DataFrame:
    ...
    # Add 2024-25 season data if available
    if include_2024_25:
        season_2024_path = os.path.join(os.path.dirname(path), "Dataset", "2024-25.csv")
        if os.path.exists(season_2024_path):
            df_2024 = pd.read_csv(season_2024_path)
            # Keep only necessary columns
            cols_to_keep = ['Div', 'Date', 'HomeTeam', 'AwayTeam', 'FTHG', 'FTAG', 'FTR']
            df_2024 = df_2024[[c for c in cols_to_keep if c in df_2024.columns]]
            # Normalize date format from DD/MM/YYYY to DD-MM-YYYY
            df_2024['Date'] = df_2024['Date'].str.replace('/', '-')
            df = pd.concat([df, df_2024], ignore_index=True)
            print(f"✓ Added {len(df_2024)} matches from 2024-25 season")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Enhanced EPL match prediction with feature engineering")
